"""Edit an image using AtlasCloud and save the output metadata to a dated results file."""
...
IMAGES_ENV = "ATLASCLOUD_EDIT_IMAGES"
...
def parse_images(raw_images: str) -> list[str]:
    if not raw_images:
        raise RuntimeError(f"Environment variable {IMAGES_ENV} is required.")
    images = [item.strip() for item in raw_images.split(",") if item.strip()]
    if not images:
        raise RuntimeError(f"Environment variable {IMAGES_ENV} must contain at least one image URL or path.")
    return images
...
def main() -> int:
    ...
    images = parse_images(get_env_value(IMAGES_ENV, required=True))
    size = detect_image_size(images[0])

    encoded_images = []
    for image_location in images:
        if is_url(image_location):
            encoded_images.append(image_location)
        else:
            image_path = Path(image_location)
            if not image_path.exists():
                raise FileNotFoundError(f"Image path not found: {image_location}")
            encoded_images.append(image_to_data_uri(image_path))
    ...
    print(f"Sending edit request to {api_url}")
    ...
    output_file = RESULTS_DIR / f"edit_{datetime.now(timezone.utc):%Y.%m.%d}.json"
    ...
    if isinstance(result.get("data", {}).get("outputs"), list):
        ...
        if save_folder is not None:
            try:
                saved_path = save_output_image(output, save_folder, prediction_id, images[0])
